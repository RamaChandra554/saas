...
document.addEventListener("click", function (event) {
  ...
  if (offcanvasInstance && offcanvasEl.classList.contains("show")) {
    const isClickInside =
      offcanvasEl.contains(event.target) ||
...
function loadCart() {
  ...
  cart.forEach(product => {
    const productTotal = parseInt(product.price.replace("₹", "")) * product.quantity;
    total += productTotal;

    const row = document.createElement("tr");
    row.innerHTML = `
      <td>
        <img src="${product.image}" alt="${product.name}" width="60" class="me-2">
        ${product.name}
      </td>
      <td>${product.price}</td>
      <td>
      ...
      <td>₹${productTotal}</td>
      <td>
        <button class="btn btn-danger btn-sm" onclick="removeFromCart(${product.id})">Delete</button>
      </td>
    `;
